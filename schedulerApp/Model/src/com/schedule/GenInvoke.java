...
//    }
    ...
    public synchronized List<SchedulePOJO> getData(SchedulePOJO sP) {
        ...
        try {
            ...
            DataSource ds = (DataSource)ctx.lookup(sP.getDbSrc());
            con = ds.getConnection();
            stmt =con.prepareCall("{call xxfnd_schedule_pkg.generate_schedule}");
            stmt.executeUpdate();
            ...
            while (rs.next()) {
                if (rs.getString("LAST_REFRESH_TIME") == null) {
                    ...
                    val =
new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rs.getString("LAST_REFRESH_TIME")));
                }
